...
static void enumerateConfigFile(std::vector<std::string> &paths) {
  ...
  const std::string program_dir = getProgramDir();
  ...
  if (!wine_program_name.empty()) {
    paths.push_back(config_dir + mangohud_dir + "wine-" + wine_program_name +
                    ".conf");
  }
}
